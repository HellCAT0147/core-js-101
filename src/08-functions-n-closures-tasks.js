 ...
 *
 */
const getPowerFunction = (e) => (x) => x ** e;
 ...
 *   getPolynom()      => null
 */
function getPolynom(...equationСoefficients) {
  return (x) => {
    if (equationСoefficients) {
      const array = equationСoefficients.slice(0);
      return array.reverse()
        .map((coef, exp) => coef * (x ** exp))
...
function logger(func, logFunc) {
  return (...pars) => {
    const parString = pars.map((par) => JSON.stringify(par)).join(',');
    logFunc(`${func.name}(${parString}) starts`);
    const result = func.apply(this, pars);
    logFunc(`${func.name}(${parString}) ends`);
    return result;
  };
}
